...
class EnsembleManager(Step):
    ...
    def setup(self):
        ...
        """
        # Link viz script
        with path('compass.landice.tests.ismip6_run.ismip6_ais_proj2300_tp_branches',  # noqa
                  'plot_branch_ensemble.py') as target:
            symlink(str(target), f'{self.test_case.work_dir}/plot_branch_ensemble.py')  # noqa
